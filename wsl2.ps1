...
function Test-WSL ()
{
    ...
    if ((Get-WindowsOptionalFeature -Online -FeatureName Microsoft-Windows-Subsystem-Linux).State -ne 'Enabled')
    {
        ...
        if ($wslinst.Restartneeded -eq $true) { Set-Variable -Name rebootWSL -Value $true -Scope Global }
    }
...
function Test-VMP ()
{
    ...
    if ((Get-WindowsOptionalFeature -Online -FeatureName VirtualMachinePlatform).State -ne 'Enabled')
    {
        ...
        if ($vmpinst.RestartNeeded -eq $true) { Set-Variable -Name rebootVMP -Value $true -Scope Global }
    }
...
function Test-Sideload ()
{
    ...
    $sideloadKeys | ForEach-Object {
        ...
        {
            if (( (Get-ItemProperty -Path $keyPath -Name $_).$_ ) -ne 1 ) {$return = $false}
        }
    }
    return $return
}
function Enable-Sideload ()
{
    ...
    $keyPath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\AppModelUnlock"
    if (!(Test-Path -Path $keyPath)) {New-Item -Path $keyPath}
    $Key = Get-Item -LiteralPath $keyPath
    $sideloadKeys = @("AllowAllTrustedApps", "AllowDevelopmentWithoutDevLicense")
    function Test-RegProperty ($propertyname)
    {
...
function Select-Distro ()
{
    ...
    {
        $installedTxt = ""
        if (($distrolist.installed)[$i]) {$installedTxt = "(Installed)"}
    ...
    {
        if (($distroChoice -gt 0) -and ($distroChoice -le $distrolist.Length)) {$choiceNum = ($distroChoice - 1)}
    }
...
function Install-Distro ($distro)
{
    ...
    function Add-WSLAppx ($distro) {
        ...
        $abortInstall = $false
        if (($distro.sideloadreqd -eq $true) -and (!(Test-Sideload))){
...
function Process ()
{
    ...
    else
    {
        ...
        else {Write-Host(" ...WSL update already installed.")}
        ...
        if ($distro.AppxName.Length -gt 1) 
        {
            if ($distro.sideloadreqd) { if (Test-Sideload) {Start-Process $distro.winpe} } 
            else { Start-Process $distro.winpe }
        } 
        else 
        {
            $wslselect = ""
